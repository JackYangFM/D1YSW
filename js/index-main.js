define(["jqueryPlugin"], function () {
	...
	$.ajax({
		type: "get",
		url: "http://localhost:8000/d1ysw-require/libs/json/banner.json",
		async: true,
	...
	function lunbo() {
		...
		$(".banner").eq(v).animate({
			...
			$(".banner").eq(currentIndex).animate({
				opacity: 1
			}, 500, function () {
	...
	$.ajax({
		type: "get",
		url: "http://localhost:8000/d1ysw-require/libs/json/topic.json",
		async: true,
		success: function (data) {
			var arr = data.datas;
			//console.log(arr)
			var dlList = "<div class='big_box clear'>";
			$.each(arr, function (index, item) {
				dlList += `<dl class="topic_box"><dt><a href="javascript:void(0)"><img src="${item.imgSrc}"/></a><div class="topic_position"></div></dt>
						<dd><a href="javascript:void(0)">${item.information}</a></dd></dl>`
			})
			$(".create_topic").append(dlList);
	...
	$.ajax({
		type: "get",
		url: "http://localhost:8000/d1ysw-require/libs/json/container.json",
		async: true,
		success: function (data) {
			...
			$(".container_sbox dl").on({
				mouseenter: function () {
					...
					$(this).find(".container_position").css({
						display: "block"
					}); //将阴影部分显示出来
					$(this).find("dd").animate({
						top: 230
					}, 200) //让当前dl下的dd改变top值进行运动
				},
				mouseleave: function () {
					$(this).removeClass("active");
					$(this).find(".container_position").css({
	...
	//创建dl的函数
	function createDl(arr, index) {
		for (var i = 0; i < arr.length; i++) {
			$('<dl><dt><img class="lazy" data-original="' + arr[i].mysrc + '" alt="" /><div class="container_position"></div></dt><dd><a class="change_color" href="javascript:void(0)">' + arr[i].info + '</a><a class="change_hide" href="javascript:void(0)">会员价：<span>' + arr[i].price + '</span></a></dd>').appendTo($(".container_sbox").eq(index));
		}
		// console.log(arr,v)
		// var dl = "<dl>";
		// $.each(arr[v],function(index,item){
		// 	//console.log(arr[v])
		// 	dl += `<dt><img class="lazy" data-original="${item.mysrc}" alt="" /><div class="container_position"></div></dt>
		// 			<dd>
		// 				<a class="change_color" href="javascript:void(0)">${item.info}</a>
		// 				<a class="change_hide" href="javascript:void(0)">会员价：<span>${item.price}</span></a>
		// 			</dd>`
		// })
		// $(".container_sbox").append(dl);
		//创建之后执行懒加载

...
function addSearch(){
	$.ajax({
		type:"get",
		url:"http://localhost:8000/d1ysw/js/search.json",
		async:true,
...
function loadSearch(){
	...
	$.ajax({
		type:"get",
		url:"http://localhost:8000/d1ysw/js/handler.php",
		data:{txt:v},
		async:true,
		success:function(data){
			var arr = eval(data);
			...
			$(".logo_search_ul").css({display:"block"});
...
function chooseLi(){
	...
	$(".logo_search_ul").css({display:"none"});
}
...
$(".nav_ul_diff").on({
	mouseenter:function(){
		...
		$(".nav_ul_div").css({display:"block"});
	},
	mouseleave:function(){
		$(".nav_ul_diff").removeClass("active");
		$(".nav_ul_div").css({display:"none"});
	}
})